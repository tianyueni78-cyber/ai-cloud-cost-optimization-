...
```text
完整性 + 唯一性 + 有效性 + 一致性 + 可关联性 + 及时性 + 业务规则
= 数据审计
```

| 审计维度 | 固定检查方法 | 随项目变化的判断依据 |
|---|---|---|
| 完整性 | 各字段空值数量和比例 | 哪些字段允许为空、何时不适用 |
| 唯一性 | 主键和业务键重复组、额外行 | 真正代表唯一事件的字段组合 |
| 有效性 | 类型转换、枚举、最小值和最大值 | 合法枚举和合理业务范围 |
| 一致性 | 大小写、分隔符、单位、跨表字段 | 标准格式和权威数据源 |
| 可关联性 | 原始与标准化后的关联率 | 主外键、时间生效条件和容许缺口 |
| 及时性 | 起止时间、缺口、延迟入库 | 刷新频率、时区和截止日期 |
| 业务规则 | 跨字段计算和逻辑约束 | 金额公式、容量、状态和合同规则 |

`sql/00`、`01`、`02` 等编号不是通用知识。通用职责依次是：认识结构、盘点数据源、做广度审计、对异常做深度分类。换数据时职责不变，但主键、允许空值、合理范围和业务规则必须重写。

标准输出是“问题、证据、影响、处理建议”四列表；完成标准不是没有异常，而是每类异常都已量化、解释，并知道是否需要清洗。
