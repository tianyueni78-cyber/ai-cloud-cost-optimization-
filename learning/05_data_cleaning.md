...
```text
审计发现 → 明确规则 → 测试规则 → 生成新数据 → 前后对账
= 可复现的数据清洗
```

| 固定原则：任何项目都适用 | 变化部分：必须由审计决定 |
|---|---|
| 原始层只读，清洗结果另存 | 哪些 ID 需要怎样标准化 |
| 每条清洗规则都有依据和验证 | 重复记录保留哪一条以及原因 |
| 空值不默认等于 0 | 保留、排除、回填还是统计填充 |
| 类型转换失败必须可见 | 日期、数值、布尔和时区字段 |
| 清洗前后进行行数、金额、关联率对账 | 哪些总量必须保持，哪些允许变化 |
| 代码可从原始数据重新生成结果 | SQL、Pandas 或数据平台的选择 |

标准输出包括清洗代码、清洗数据和质量汇总；完成标准是所有变化都能对应审计规则，关键不变量通过验证，原始数据没有被覆盖。

本项目使用 Pandas 和 CSV 是具体实现，不是唯一公式。换成数据库时可以用 SQL 视图，数据更大时可以用 Parquet 或数据仓库；固定不变的是“规则有依据、处理可重复、结果可对账”。
